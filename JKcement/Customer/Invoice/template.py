# Finance/Accounting/template.py
# -------------------------------------------------------------
# TEMPLATE ENGINE \u2014 Brown Theme (Production-Ready)
# -------------------------------------------------------------
...
<head>
    ...
    <style>
        ...
        tr:hover { background: #F1FFE1; }

        /* Sliders */
        .date-slider-wrapper .slider-container { background: #D8F3DC!important; border-radius: 10px; }
    ...
    <div class="main">
        <div class="top-nav">
            <div>
                ...
                <h4 id="exc-title" style="margin: 5px 0 0 0; font-size: 13px; color: #4F6F52; font-weight: 700; text-transform: uppercase; opacity: 0.8;"></h4>
            </div>
        ...
        <div class="table-box">
            <div class="table-header-flex">
                <h3 style="font-size:15px;margin:0;font-weight:800;color:var(--sidebar);">Detailed Transaction Log</h3>
...
<script>
        const defaultCards = [
            ...
            { id: 'k-cust', label: 'Customers', agg: 'unique', source: 'customer' },
        ...
        const defaultFilters = [
            ...
            { id: 'sel-cust', label: 'Customer', source: 'customer' },
            { id: 'sel-city', label: 'City', source: 'city' }
        ];
        const defaultCharts = [
            { id: 'chartPie', type: 'pie', x: 'customer', agg: 'count', top_n: 5, title: 'TOP 5 CUSTOMERS (RECORD COUNT)', legend: true },
...
const CC = ['#3E5F44','#6CA651','#97D991','#5E936C','#A5D6A7','#2E4A35','#7FB069'];
...
fill: type === 'line' ? {target: 'origin', above: 'rgba(62,95,68,0.08)'} : false,                tension: 0.4
            }]
        },
        options: {
            responsive: true, maintainAspectRatio: false,
            plugins: {
                title: { display: true, text: cfg.title || (cfg.y ? `${cfg.y} by ${cfg.x}` : `${cfg.x} Distribution`).toUpperCase(), font: { size: 13, weight: '800' }, color: '#000' },
...
function popSlicers() {
    (curExcFilters || []).forEach(f => {
        ...
        const allLbl = f.all_label || `All ${f.label}`;
        sel.html(`<option value="ALL">${allLbl}</option>`);
        if (col && rawD.length > 0) { [...new Set(rawD.map(r => r[col]))].filter(x => x).sort().forEach(v => sel.append(`<option value="${v}">${v}</option>`)); }
    });
}
...
function selObj(id, name, el) {
    ...
    // Initial UI build from first exception or defaults
