# Finance/Spend_Analysis/loader.py
...
import Finance.Accounting.template as base_template

def get_live_configs(category_folder):
    cat_dir = os.path.dirname(__file__)
    insight_files = sorted(glob.glob(os.path.join(cat_dir, 'FJ*.py')))
    insight_configs = {}
    insight_modules = {}
    objectives = []
    ...
    for fpath in insight_files:
        ...
        full_module = f"Finance.{category_folder}.{module_name}"
        try:
...
def create_blueprint(bp_name, page_title, url_prefix, category_folder):
    ...
    @bp.route('/')
    def dashboard():
        importlib.reload(base_template)
    ...
    def api(oid, exc):
        ...
        try:
            ...
            if df is None or df.empty:
                return jsonify({"success": False, "msg": "No data found for this exception"})
