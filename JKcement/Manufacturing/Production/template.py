...
<head>
    ...
    <style>
        ...
        .side-item { position:relative; padding:12px 20px; margin:4px 0; border-radius:30px 0 0 30px; cursor:pointer; font-size:11px; color:#F1948A; transition:all 0.2s ease; font-weight:600; }
        ...
        .kpi-lbl { font-size:10px; color:#F1948A; text-transform:uppercase; font-weight:700; margin-top:8px; opacity:0.9; }
        ...
        .date-slider-wrapper .slider-container { background:#F5CEC7!important; border-radius:10px; }
    ...
    <div class="main">
        <div class="top-nav">
            <div>
                ...
                <h4 id="exc-title" style="margin: 5px 0 0 0; font-size: 13px; color: #7F8C8D; font-weight: 700; text-transform: uppercase; opacity: 0.8;"></h4>
            </div>
...
const CC = ['#2C3E50','#E74C3C','#7F8C8D','#C0392B','#34495E','#F1948A','#E67E22'];
...
function updateKPIs(){const allFilt=isAllFilters();(curExcCards||[]).forEach(card=>{let val=0;if(!filteredD||filteredD.length===0){$('#'+card.id).text(card.format==='currency'?'\u20b90':(card.format==='percentage'?'0.00%':'0'));return;}const sourceCol=card.source?rCol(card.source):null;if(card.agg==='row_count')val=filteredD.length;else if(card.agg==='total_rows')val=(bStats&&allFilt)?bStats.total_rows:filteredD.length;else if(card.agg==='total_value'){if(bStats&&allFilt)val=bStats.total_value;else if(sourceCol)val=filteredD.reduce((a,b)=>a+parseNum(b[sourceCol]),0);}else if(card.agg==='unique'){val=sourceCol?new Set(filteredD.map(r=>r[sourceCol]).filter(v=>v!=='')).size:0;}else if(card.agg==='avg'){if(filteredD.length===0)val=0;else{let sum=sourceCol?filteredD.reduce((a,b)=>a+parseNum(b[sourceCol]),0):0;val=sum/filteredD.length;}}let displayVal='';if(card.format==='currency'){displayVal=fmt(val,true);}else if(card.format==='percentage'){displayVal=val.toFixed(2)+'%';}else if(card.agg==='avg'){displayVal=val.toFixed(1);}else{displayVal=val.toLocaleString('en-IN');}$('#'+card.id).text(displayVal);});}
function updateCharts(){if(!curExcCharts||curExcCharts.length===0)return;curExcCharts.forEach(cfg=>{const box=$('#box-'+cfg.id);box.find('.no-data-msg').remove();if(cInst[cfg.id])cInst[cfg.id].destroy();const xCol=rCol(cfg.x),yCol=rCol(cfg.y);if(!xCol||!filteredD.length){handleChartError(cfg.id,box);return;}let data={};if(cfg.time_group==='month'){filteredD.forEach(r=>{const o=new Date(r[xCol]);if(!isNaN(o)){const k=o.toLocaleString('default',{month:'short',year:'numeric'});data[k]=(data[k]||0)+(yCol?parseNum(r[yCol]):1);}});if(cfg.agg==='avg'&&yCol){let counts={};filteredD.forEach(r=>{const o=new Date(r[xCol]);if(!isNaN(o)){const k=o.toLocaleString('default',{month:'short',year:'numeric'});counts[k]=(counts[k]||0)+1;}});Object.keys(data).forEach(k=>{data[k]=data[k]/(counts[k]||1);});}const sk=Object.keys(data).sort((a,b)=>new Date(a)-new Date(b));data=Object.fromEntries(sk.map(k=>[k,data[k]]));}else{let counts={};filteredD.forEach(r=>{const k=r[xCol]||'N/A';data[k]=(data[k]||0)+(cfg.agg==='count'?1:parseNum(r[yCol]));counts[k]=(counts[k]||0)+1;});if(cfg.agg==='avg'&&yCol){Object.keys(data).forEach(k=>{data[k]=data[k]/(counts[k]||1);});}if(cfg.top_n)data=Object.fromEntries(Object.entries(data).sort((a,b)=>b[1]-a[1]).slice(0,cfg.top_n));}const labels=Object.keys(data),values=Object.values(data);if(!labels.length){handleChartError(cfg.id,box);return;}$('#'+cfg.id).show();renderChartJS(cfg,labels,values);});}
