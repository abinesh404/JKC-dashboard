...
CONFIG = {
    ...
    "columns": {
        ...
        "department": ["Department","Cost Center","Manager Name"]
            ...
            "cards": [{'id': 'k1', 'label': 'New Joiner Early Claims', 'agg': 'total_value', 'source': 'amount', 'format': 'currency'}, {'id': 'k2', 'label': 'New Joiner Claimants', 'agg': 'unique', 'source': 'employee'}, {'id': 'k3', 'label': 'Early Claims Count', 'agg': 'total_rows'}, {'id': 'k4', 'label': 'Affected Cost Centers', 'agg': 'unique', 'source': 'department'}],
    "filters": [
        {"id": "f1", "label": "Companies", "source": "company"},
        {"id": "f2", "label": "Employees", "source": "employee"},
        {"id": "f3", "label": "Departments", "source": "department"}
    ],
    "charts": [{'id': 'c1', 'type': 'bar', 'x': 'department', 'y': 'amount', 'agg': 'sum', 'title': 'NEW JOINER EARLY CLAIMS BY DEPT'}, {'id': 'c2', 'type': 'doughnut', 'x': 'company', 'agg': 'count', 'title': 'EARLY CLAIMS BY COMPANY AREA'}, {'id': 'c3', 'type': 'line', 'x': 'date', 'y': 'amount', 'agg': 'sum', 'time_group': 'month', 'title': 'EARLY CLAIMS TREND'}]
}
...
def get_data(exc_id):
    paths = [
        rf"data_files/PJPA29_Exception{int(exc_id):02}.csv"
    ]
