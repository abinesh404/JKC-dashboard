...
function updateKPIs(){const allFilt=isAllFilters();(curExcCards||[]).forEach(card=>{let val=0;if(!filteredD||filteredD.length===0){$('#'+card.id).text(card.format==='currency'?'\u20b90':(card.format==='percentage'?'0.00%':'0'));return;}const sourceCol=card.source?rCol(card.source):null;if(card.agg==='row_count')val=filteredD.length;else if(card.agg==='total_rows')val=(bStats&&allFilt&&!card.source)?bStats.total_rows:filteredD.length;else if(card.agg==='total_value'){if(bStats&&allFilt&&card.source==='amount')val=bStats.total_value;else if(sourceCol)val=filteredD.reduce((a,b)=>a+parseNum(b[sourceCol]),0);}else if(card.agg==='sum'){val=sourceCol?filteredD.reduce((a,b)=>a+parseNum(b[sourceCol]),0):0;}else if(card.agg==='unique'){val=sourceCol?new Set(filteredD.map(r=>r[sourceCol]).filter(v=>v!=='')).size:0;}else if(card.agg==='avg'){if(filteredD.length===0)val=0;else{let sum=sourceCol?filteredD.reduce((a,b)=>a+parseNum(b[sourceCol]),0):0;val=sum/filteredD.length;}}else if(card.agg==='ratio'){if(!rawD||rawD.length===0)val=0;else{let sum=sourceCol?filteredD.reduce((a,b)=>a+parseNum(b[sourceCol]),0):0;let total=sourceCol?rawD.reduce((a,b)=>a+parseNum(b[sourceCol]),0):0;val=total===0?0:(sum/total)*100;}}let displayVal='';if(card.format==='currency'){displayVal=fmt(val,true);}else if(card.format==='percentage'){displayVal=val.toFixed(2)+'%';}else if(card.agg==='avg'){displayVal=val.toFixed(1);}else{displayVal=val.toLocaleString('en-IN');}$('#'+card.id).text(displayVal);});}
