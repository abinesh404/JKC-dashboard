# Supplier/master_data/loader.py
...
def get_live_configs(category_folder):
    ...
    for fpath in insight_files:
        ...
        full_module = f"Supplier.{category_folder}.{module_name}"

        try:
            if full_module in sys.modules:
                mod = importlib.reload(
                    sys.modules[full_module]
                )
            else:
...
def create_blueprint(
    ...
    def dashboard():
        ...
        insight_configs, _, objectives = get_live_configs(
            category_folder
        )
    ...
    def api(oid, exc):
        insight_configs, insight_modules, _ = get_live_configs(
            category_folder
        )
        ...
        try:
            ...
            df.columns = [
                str(c).strip().replace('\xef\xbb\xbf', '').replace('\ufeff', '')
                for c in df.columns
            ]

            amt_candidates = config.get(
                'columns',
                {}
            ).get('amount', [])
