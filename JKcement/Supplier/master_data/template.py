    ...
    'SJPR9': {
        ...
        function selObj(id, name, el) {
            ...
            if (aCfg && Object.keys(aCfg).length > 0) {
                ...
                buildExcBtnsStatic();
                handleMergedInsightsUI(id); buildKPIs(defaultCards);
        ...
        function populateSlicers() {
            curExcFilters.forEach(f => {
                ...
                if (f.id === 'sel-extype') { (aCfg.active_exceptions || []).forEach(e => { const label = e.label; const mappedName = (typeof EXC_NAMES !== 'undefined' && EXC_NAMES[curID] && EXC_NAMES[curID][label]) || label; sel.append('<option value="' + label + '">' + mappedName + '</option>'); }); } else if (col && rawD.length > 0) {
